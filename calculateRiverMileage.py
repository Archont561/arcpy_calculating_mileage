...
WORKSPACE = Path(r"#PODAJ ŚWIEŻKĘ DO OBSZARU ROBOCZEGO!!!")
...
def distance_from_begining(
    ...
    with (arcpy.da.SearchCursor(lines, [line_id_field, "SHAPE@"]) as line_cursor,
        arcpy.da.InsertCursor(distances_table, [line_id_field, origin_id, distance_field]) as table_cursor):
...
def calculate_rivernet_mileage(
    ...
    if not river_id_field:
        river_id_field = "RiverID"
        arcpy.management.CalculateField(
            in_table=rivernet,
            field=river_id_field,
            field_type="TEXT",
            expression=f"!FID!",
            expression_type="PYTHON3"
        )
    ...
    intersections_with_riverids = arcpy.analysis.SpatialJoin(
        target_features=intersections,
        join_features=rivernet,
        out_feature_class="rivers_intersections_join",
        join_operation="JOIN_ONE_TO_ONE",
        join_type="KEEP_COMMON",
        field_mapping=rf'{river_id_field}_Join "{river_id_field}" true true false 255 Text 0 0,Join,",",{rivernet},{river_id_field},0,253',
    ...
    intersections_mileages = distance_from_begining(
        rivernet, intersections_with_riverids, river_id_field, distance_field)
    with arcpy.da.UpdateCursor(intersections_mileages, 
        [river_id_field, "ORIG_FID", distance_field]) as intersections_cursor:
        inter_mileages = dict()
        for river_id, origin_id, distance in intersections_cursor:
            if origin_id not in inter_mileages:
                inter_mileages[origin_id] = {"main": river_id, "distance": distance}
            else:
                if distance > inter_mileages[origin_id]["distance"]:
                    inter_mileages[origin_id] = {
                        "main": river_id, 
                        "tributary": inter_mileages[origin_id]["main"], 
                        "distance": distance
                    }
                else: inter_mileages[origin_id]["tributary"] = river_id
        inter_mileages = [(v["distance"], v["main"], v["tributary"]) for v in inter_mileages.values()]

    for path in [intersections, intersections_with_riverids, intersections_mileages]: 
        arcpy.management.Delete(path)

    features_to_locate_mileages = distance_from_begining(
        rivernet, features_to_locate, river_id_field, distance_field, tolerance=tolerance)    
    ...
    with arcpy.da.UpdateCursor(features_to_locate_mileages,
        ["OID@", distance_field, river_id_field, full_distance_field, path_field],
        ...
        for fid, distance, river_id, _, _ in points_cursor:
            ...
            while True:
                ...
                current_path.append(next_river_id)
...
def points_from_mileage(
    ...
    with (arcpy.da.SearchCursor(rivernet, [river_id_field, "SHAPE@"]) as current_river_cursor,
...
def main():
    with arcpy.EnvManager(workspace=str(WORKSPACE), overwriteOutput=True):
        ...
        #     river_id_field="RiverID",
        # )
        # points_from_mileage("distance_table", "rivers.shp", "RiverID", "Distance")
        # generate_points_along_line("rivers.shp", 5)
        print("END")
